...
"""
Run SPEFit on all hits, only DC hits (DC) and only Ice Cube hits (NoDC)
"""
...
from icecube.filterscripts.offlineL2.Globals import (
    which_split, deepcore_wg, muon_wg, wimp_wg, cascade_wg, fss_wg,
    fss_wg_finiteReco, ehe_wg, ehe_wg_Qstream
)
...
def SPE(
    ...
    if N_iter > 1:
        tray.AddSegment(
            lilliput.I3IterativePandelFitter,
            SPEFit + suffix,
            pulses=Pulses,
            n_iterations=N_iter,
    ...
    tray.AddModule(
        'CramerRao', name + '_' + SPEFitCramerRao + suffix,
...
def DCPulses(omkey, index, pulse):
    return (
        omkey.string > 78 and omkey.om > 10
        or (omkey.om > 40 and omkey.string in [26, 27, 45, 46, 35, 36, 37])
    )


def NoDCPulses(omkey, index, pulse):
    return not DCPulses(omkey, index, pulse)
