...
def get_bin_index(t, x, y, z):
    ...
    r_index = int(math.floor((radius * 100)**0.5))
    if radius == 0:
        theta = 0
    else:
        theta = np.arccos(z / radius)
    theta_index = int(math.floor(theta * 40 / np.pi))
    phi = np.arctan2(y, x)
    phi_index = int(math.floor(phi * 10 / np.pi))
...
def get_track_lengths(t, x, y, z, theta, phi, total_track_length):
    ...
    t_bin_edges = np.linspace(0, 3000e-9, n_t_bins+1)
    r_bin_edges = PowerAxis(0, 400, n_r_bins, 2)
    ...
    while cumulative_track_length < total_track_length and radius < 400:
