...
def plot_2d(data, bin_edges, lables, ax, x_idx, y_idx, log=False, cmap='CMRmap_r', cb=True, vmin=None, vmax=None):
    idx = range(data.ndim)
    idx.remove(x_idx)
    idx.remove(y_idx)
    z = data.sum(axis=tuple(idx))
    if log:
        if vmin is None: vmin = np.log(z[z != 0].min())
        if vmax is None: vmax = np.log(z.max())
        z[z == 0] = vmin
...
def plot_1d(data, bin_edges, lables, ax, x_idx, log=False):
    ...
    ax.hist(0.5*(bin_edges[x_idx][:-1] + bin_edges[x_idx][1:]), weights=y, bins=bin_edges[x_idx], histtype='step', lw=1.5)
    ax.set_xlim([bin_edges[x_idx][0],bin_edges[x_idx][-1]])
    if log:
        ax.set_yscale('log')
...
for tidx in range(r_cos_t_data.shape[2]):
    im = plot_2d(r_cos_t_data[:,:,tidx], bin_edges[:2], lables[:2], ax1, 1, 0, log=True, vmax=vmax, vmin=vmin, cb=False)
    ims.append([im])
