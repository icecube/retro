...
More technical details of how the method works will follow, but now we look at the benefits and weaknesses of Retro Reco as it is currently implemented.

### Benefits and weaknesses of Retro Reco table generation
...
* DOM and cable geometries are not considered for photon absorption and reflection, but should be possible to include in the future within `clsim` as (1) a more complicated light sources and (2) object geometries that photons can interact with. These things might be challenging to implement in `clsim`, but there is nothing about Retro itself that precludes including these.
...
* The individual-DOM tables needn't be modified, but the whole-detector time- and DOM-independent table used for computing no-hit likelihoods for each unique detector geometry (i.e., each unique GCD). This takes on the order of a few hours to do on a small cluster.

### Benefits and weaknesses of Retro Reco event hypotheses

Event hypotheses in Retro Reco are highly modular and _completely_ independent of the Retro tables (i.e., tables needn't be regenerated even for a completely new type of hypothesis that was not considered previously).
...
The Retro tables take care of connecting the photons generated by the event hypothesis photons to the number and timing of expected hits at each DOM.
...
* Time to evaluate is proportional to the number of samples from the hypothesis, so using the same time step, high energy events will be slower to reconstruct than low energy events. The time step can be modified, though, and it would be straightforward to implement dynamic time steps; no software changes need to be made outside the muon and/or cascade kernel functions to make this work.
