...
for cluster_idx in range(80):
    print 'table cluster %s'%(cluster_idx)
    table_5d = np.load('/data/icecube/retro_tables/tilt_on_anisotropy_on_noazimuth_80/cl%s/ckv_table.npy'%(cluster_idx))
    print 'table loaded'
    # create 3d table
    ...
    print 'PCA fitted'
    with open('pca_after_cl%s.pkl'%(cluster_idx),'wb') as f:
        pickle.dump(pca, f)
    del table_5d_normed
    del table_3d
    del table_5d
